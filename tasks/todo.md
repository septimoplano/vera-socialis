...
- [ ] **A1 · Prototipo visual actualizado** (E1 → CP1)
...
- [ ] **B2 · Esquema de datos completo + seed** (E2)
...
- [ ] **AB1 · Contrato API v1** (ambos equipos, cto-cio arbitra)
...
- [ ] **B3 · Auth: registro + referido + sesiones + passkey** (E3)
...
- [ ] **B4 · Selfie pipeline + panel admin** (E3)
...
- [ ] **B5 · Staging temprano: Cloud Run + Neon + R2 + CI/CD** (E8 adelantado; requiere dominio del fundador)
...
- [ ] **B7 · Sentimiento + motor de score + moderación** (E4)
...
- [ ] **B8 · WS: chat ciphertext + media + señalización WebRTC** (E5)
...
- [ ] **B9 · Comunidad + empresas + tickets** (E7)
...
- [ ] **A3 · Base React PWA + sistema visual** (post-CP1)
...
- [ ] **A4 · Registro/login UI** — referido, passkey, selfie con cámara, estados de espera de aprobación, botón discreto empresas.
  - Verify: flujo completo sobre mocks; luego real contra B3-B5.
  - Files: `web/src/vistas/auth/*`

- [ ] **A5 · Perfil + métricas personales** — banner, grid, medalla, nivel, tarjetas categorías, público/privado, panel privado de métricas con feedback por sesión.
  - Verify: checklist §2.2; métricas cambian tras sesión simulada.
  - Files: `web/src/vistas/perfil/*`

- [ ] **A6 · Mi Red** — solicitudes, buscador por nombre, grafo canvas tipo red neuronal.
  - Verify: grafo con 20 nodos fluido en móvil.
  - Files: `web/src/vistas/red/*`

- [ ] **A7 · Contenido mundial + Socialis** — categorías, scroll por categoría, posteo, propaganda VERA cada 30, feed cronológico de conexiones, guardar.
...
- [ ] **A8 · Comentarios + notificaciones UI** — comentar (sin ver clasificación), pantalla notificaciones §2.10 con badge verde y navegación por tap.
  - Verify: checklist §2.10 completo.
  - Files: `web/src/vistas/notificaciones/*`, componentes comentarios

- [ ] **A9 · Chat UI + E2E cliente** — claves WebCrypto en dispositivo, conversaciones, texto/archivos/audio/ubicación, reacciones ocultas.
  - Verify: **CP3 parte 1 — mensaje E2E entre 2 dispositivos reales vía staging.**
  - Files: `web/src/vistas/chat/*`, `web/src/crypto/*`

- [ ] **A10 · Llamadas WebRTC UI** — llamada y videollamada con señalización WS.
  - Verify: **CP3 parte 2 — videollamada entre 2 celulares reales.**
  - Files: `web/src/vistas/chat/llamadas*`

- [ ] **A11 · Mecánicas de bienestar** (100% frontend — colchón anti-bloqueo)
...
- [ ] **A12 · Comunidad + Config UI** — votaciones, buzón, config completa (§2.9), ayuda con tickets.
  - Verify: checklist §2.8-2.9.
  - Files: `web/src/vistas/comunidad/*`, `web/src/vistas/config/*`

- [ ] **A13 · QA final + Playwright** — flujos críticos automatizados (registro, mensaje E2E, posteo con categoría), checklist 9 secciones en 2 celulares, verificación visual contra prototipo.
  - Verify: **CP4 — criterios de éxito spec §12 completos.**
  - Files: `e2e/*`, reporte en `docs/`

## Cierre

- [ ] **AB2 · Beta live** — onboarding de los primeros 10 usuarios del círculo con sus códigos de referido; informe de cierre.
